...
> `lib/llm.ts` が環境変数からプロバイダを決めるため、**Azure OpenAI / Google Gemini** を
...
AIを呼ぶには `.env.local` にキーが要る（**キー無しではAPIがエラーを返す**。キー未設定時のモックモードは廃止した）。
...
判定カードの見た目だけ確認するなら `/preview?open=red`、AIを呼ばずに撮影の流れを通すなら `/?demo=red` を使う（固定応答）。
...
| AI | Azure OpenAI プロキシ（`gpt-5.1`）／ Google Gemini（`gemini-3.5-flash` / `gemini-3.6-flash`）。Vision + 構造化出力、失敗時は自動フォールバック |
...
lib/llm.ts              AIプロバイダの抽象とフォールバック（Azure / Gemini）
