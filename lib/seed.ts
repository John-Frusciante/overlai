 ...
 * デモ用シードデータ — 設計仕様書 §6.2
 *
 * stk-011（テトラサイクリン系抗菌薬）は吸収阻害の判定を実演するために置いている。
 * 鉄・カルシウム・マグネシウムを含むサプリをスキャンすると、この薬の吸収が
 * 妨げられる可能性を指摘できる（企画書 §6 の機能一覧9番）。
 *
 * ⚠ ここをニューキノロン系（レボフロキサシン等）に戻さないこと。
 * キノロン系はNSAIDsとの併用注意が添付文書にあり、相互作用の判定（lib/knowledge.ts）を
 * 入れた結果、市販イブプロフェン製剤のスキャンが🔴の条件にも該当してしまう。
 * デモの主役である🟡判定を安定させるため、吸収阻害だけが効くテトラサイクリン系にしてある。
 *
