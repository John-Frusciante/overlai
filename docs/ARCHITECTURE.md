...
| 実装規模 | TypeScript 1,256行（`app/` `components/` `lib/`） |
...
```
app/
  page.tsx              マイストック画面（クライアント）
  scan/page.tsx         スキャン画面（カメラ・画像選択・ローディング・エラー）
  preview/page.tsx      判定カードのプレビュー（開発用）
  api/analyze/route.ts  判定API（サーバー・APIキーを保持する唯一の場所）
  layout.tsx            メタデータ・ビューポート
  globals.css           Tailwind + 日本語フォントスタック

components/
  StockList.tsx         在庫一覧（カテゴリ別・処方バッジ）
...
lib/
  ...
  seed.ts               デモ用シードデータ7件
  storage.ts            localStorage アクセスの集約点
  image.ts              送信前リサイズ
...
| 在庫の追加・編集UI | 未実装（判定の証明に不要なため意図的に捨てた） |
| JAHIS QR・レシート登録 | 未実装（同上） |
| 塗る順序ナビ | 未実装（Issue #12・優先度低） |
