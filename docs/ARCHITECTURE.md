...
components/
  ...
  StockActionSheet.tsx  項目タップで開く操作シート（編集／削除）
...
| | 内容 |
| :--- | :--- |
| 入力 | 区分ごとの確定済みの並び（id・商品名・剤形・成分）、肌質・頭皮の申告と自由記述、服用中の薬 |
...
| **項目をタップ** | 下から操作シート（`StockActionSheet`）が上がり、編集・削除を選ぶ。**これが主の導線** |
| 「編集」ボタン | 編集モード。各行に編集（`/stock/new?id=`）と削除のボタンが並ぶ。複数件を続けて消すとき用に残している |
| 削除 | どちらの導線も確認をはさむ二段階。localStorage から復元できないため |

カードは全面がタップ領域になる。内側に `Link` を持つため入れ子のボタンにはできず、
**透明なボタンを重ね、編集モード中のボタンだけ `z-10` で前面に置いて**両方の導線が競合しないようにしている。
