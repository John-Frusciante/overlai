...
**#11 順序ソート** — 並び順は剤形の重みで決めるルールベース（`lib/routine.ts`）。**その上に、在庫の顔ぶれと肌質・頭皮に応じた一言をAIが足す**（`POST /api/routine`）。順序と言葉を切り離しているので、AIが落ちても並び順と理由は出続ける（[ARCHITECTURE.md §4.6](ARCHITECTURE.md)）。
...
ルーティンの解説のAI生成（`POST /api/routine`・順序はルールのまま）
