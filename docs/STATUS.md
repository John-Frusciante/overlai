...
| 更新日 | 2026年9月15日（エントリー完了。#23 障害時のUIを確認して4件修正。Anthropic 経路とモックモードを廃止） |
| :--- | :--- |
...
プロバイダは `lib/llm.ts` が環境変数から決める。**Azure OpenAI / Google Gemini の2経路**があり、コード変更なしで切り替わる。
（Anthropic 経路とキー未設定時のモックモードは 2026年9月15日に廃止。固定応答は `?demo=` と `/preview` だけ）
...
**#11 順序ソート** — 区分は組み込みの2つに加えて**ユーザーが「朝のスキンケア」などを作れる**（カテゴリと同じ流儀）。並び順は剤形の重みで決めるルールベース（`lib/routine.ts`）。**その上に、在庫の顔ぶれと肌質・頭皮に応じた一言をAIが足す**（`POST /api/routine`）。順序と言葉を切り離しているので、AIが落ちても並び順と理由は出続ける（[ARCHITECTURE.md §4.6](ARCHITECTURE.md)）。**順番はユーザーが手で入れ替えられる**（剤形どおりに戻す導線つき）。

**#13 洗浄基剤 × 肌質** — 実装済み。成分名から洗浄基剤の系統（アミノ酸系／ベタイン系／高級アルコール系／石鹸系）を判定し、肌質・頭皮の自己申告と突き合わせる。**AIを使わずルールベースなので、キーが無くても本物として動く。** 肌質・頭皮の全5値／全4値に対して判定が出る（以前は頭皮×石鹸系と「普通」「混合」が抜けていた）。
...
### 障害時のUIの確認で見つかったもの（2026年9月15日・#23）

「実装してあるが起こして確かめていない」領域を潰したところ、**4件が実際に壊れていた**。

| # | 問題 | どう直したか |
| :---: | :--- | :--- |
| 1 | **上流が応答しないと 61.5秒 かかり、Gemini の保険が動かない** | OpenAI SDK が既定でタイムアウトも2回やり直すため 20秒×3 になっていた。`maxRetries: 0` にして 20.1秒で次へ落ちるようにした（やり直しは `attempt()` に集約） |
| 2 | **本番では `?demo=` が効いていなかった** | キー未設定のモックモードでしか見ていなかった。キーがあっても固定応答を返し、カードに「デモ用の固定応答です」と出す（同日、モックモード自体は廃止） |
| 3 | 読み込み画面のまま固まりうる（504 の本文が JSON でない／応答が来ない） | 状態コードから文言を組み立て、70秒で打ち切る。オフラインは「オフラインです」と区別する |
| 4 | 読めない画像を選ぶと何も起きない | `createImageBitmap` の例外が未処理だった。「画像を読み込めませんでした」を出す |

加えて、入口の回数制限が IP 単位で、会場では展示端末が全部同じ相手になることが分かった。
上限を `GUARD_MAX_REQUESTS` で広げられるようにした（既定 30 は変えない。展示中だけ 120）。
...
**2026年9月15日にエントリーを完了した。書類審査の結果は9月21日頃に届く。**
通過が分かってから動くと本選まで10日しかないので、結果を待つ間に `must` から着手する。
...
| 2 | `must` | Gemini 無料枠の 5 RPM をどう扱うか決める | フォールバック中に来場者が並ぶと 429 になる。課金するか、待たせる文言を出すか。#23 の確認と同時に決める | 〜9/21 | — |
| 3 | `should` | ブース展示用のワンタップ初期化 | 前の来場者の在庫が残ると、次の人に見せる判定が変わる。実装は小さい | 〜9/24 | [#25](https://github.com/John-Frusciante/overlai/issues/25) |
...
- [ ] 会場の回線が使えない場合に備え、スマホのテザリングで動くことを事前に確かめる
- [ ] 展示のあいだ Vercel に `GUARD_MAX_REQUESTS=120` を入れる（終わったら消す）
- [ ] 実機を機内モードにして「オフラインです」が出ることを1回見る
- [ ] 実機を2台以上とモバイルバッテリーを持っていく（1台が固まっても展示を止めない）
- [ ] 🟡・🔴の2パターンを再現できる実物の商品を持参する（TESTING.md の検証商品から選ぶ）
- [ ] 判定が返らないときに `?demo=` の固定応答へ切り替える手順を紙で持っておく
- [ ] 展示の合間に初期化する手順を、説明役以外のメンバーも覚えておく（#25 の導線）
- [ ] ピッチ資料と、CONTEST.md §8「聞かれそうなことへの答え」を全員で共有する
...
ルーティンまわりはブラウザで**全項目の動作確認済み**（TESTING.md §G-1〜§G-7）。
区分の追加・削除、肌質の自由記述、年代・性別まで含めて通しで確認した。
自由記述と年代・性別は一言にだけ効き、**判定の色は変えない**ことも見ている。
解説の生成に失敗しても、順序・理由・洗浄料の相性は表示され続ける。
...
| [本選](https://github.com/John-Frusciante/overlai/milestone/3) | 2026-10-01 | 進行中 | エントリー完了（9/15）・結果連絡は9/21頃。#24 #25 — ブース展示と連続デモに耐えるための整備（#20 は Anthropic 廃止で閉じ、#23 は完了） |
...
| 内容 | なぜ未検証か |
| :--- | :--- |
| エラー時のUI（実機の機内モード） | WebKit の offline エミュレーションでは画像の読み込みまで失敗するため、`navigator.onLine` を偽装して確認した（TESTING.md §K-2f）。実機では未確認 |
