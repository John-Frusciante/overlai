...
**SDK の自動再試行も切ること（`maxRetries: 0`）。** OpenAI / Anthropic の SDK は既定で2回やり直し、タイムアウトも対象になる。20秒に縮めても 3回で 60秒に戻ってしまい、実測で応答しない上流に 61.5秒かかった（2026年9月15日）。やり直しの判断は `attempt()` だけが持つ。

判定シナリオは `?demo=yellow|red|blue` で固定できる。マイストックからスキャン画面へ引き継がれるため、撮影の流れを止めずにシナリオを固定できる。**キーがあっても効く**（展示中にAIが全滅したときの逃げ道）。固定応答であることはカードに表示される。
...
| 判定が毎回同じ | モックモードか、URLに `?demo=` が残っている。カードに「デモ用の固定応答です」と出ていればこれ |
